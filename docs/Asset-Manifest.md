...
| `held-frame-spine-dusk` | The Single Held Frame | `/publishing` · `.held-frame` | 16:9 | Poster-first still, muted | same | Full-width still | AVIF/WebP (+ optional muted loop on intent) | ≤ 320 KB | `public/media/held-frame-w27.poster.svg` (on-brand placeholder) | Per `held-frame/*.json` `media.alt` | **Placeholder (SVG poster wired)** |
